...
public class ApimartVideoProvider extends AbstractVideoProvider {

    private static final String DEFAULT_BASE_URL = "https://api.apimart.cn";

    public ApimartVideoProvider(Config config, OkHttpClient httpClient, ObjectMapper objectMapper) {
        super(config, httpClient, objectMapper);
    }

    @Override
    public String createTask(VideoRequest request) {
        String url = getBaseUrl(DEFAULT_BASE_URL) + "/v1/video/generations";
    ...
    public VideoTaskResult pollTask(String taskId) {
        String url = getBaseUrl(DEFAULT_BASE_URL) + "/v1/video/generations/" + taskId;
        try {
            ...
            throw new BizException(ErrorCode.AI_CALL_FAILED, "Apimart video poll error: " + e.getMessage());
        }
    }
    ...
    private String doPostAndGetTaskId(String url, ObjectNode body) {
        try {
            ...
            try (Response resp = httpClient.newCall(req).execute()) {
                String respBody = resp.body() != null ? resp.body().string() : "";
                if (!resp.isSuccessful()) throw new BizException(ErrorCode.AI_CALL_FAILED, "Apimart video HTTP " + resp.code());
                JsonNode root = objectMapper.readTree(respBody);
                return root.path("data").path("task_id").asText();
            }
        } catch (BizException e) { throw e; }
        catch (IOException e) { throw new BizException(ErrorCode.AI_CALL_FAILED, "Apimart video error: " + e.getMessage()); }
    }
