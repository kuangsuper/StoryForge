...
@Slf4j
public class ViduImageProvider extends AbstractAsyncImageProvider {

    private static final String DEFAULT_BASE_URL = "https://api.vidu.com";

    public ViduImageProvider(Config config, OkHttpClient httpClient, ObjectMapper objectMapper) {
    ...
    protected String pollTask(String taskId) {
        ...
        try {
            ...
            try (Response resp = httpClient.newCall(req).execute()) {
                ...
                String status = root.path("state").asText("");
                if ("completed".equals(status) || "success".equals(status)) {
                    return root.path("data").path("image_url").asText(null);
                }
                if ("failed".equals(status)) {
                    throw new BizException(ErrorCode.AI_CALL_FAILED, "Vidu image task failed");
                }
                return null;
            }
        } catch (BizException e) { throw e; }
        catch (IOException e) { throw new BizException(ErrorCode.AI_CALL_FAILED, "Vidu poll error: " + e.getMessage()); }
    }
    ...
    private String doPostAndGetTaskId(String url, ObjectNode body) {
        try {
            ...
            try (Response resp = httpClient.newCall(req).execute()) {
                String respBody = resp.body() != null ? resp.body().string() : "";
                if (!resp.isSuccessful()) throw new BizException(ErrorCode.AI_CALL_FAILED, "Vidu image HTTP " + resp.code());
                JsonNode root = objectMapper.readTree(respBody);
                return root.path("data").path("task_id").asText();
            }
        } catch (BizException e) { throw e; }
        catch (IOException e) { throw new BizException(ErrorCode.AI_CALL_FAILED, "Vidu image error: " + e.getMessage()); }
    }
}
