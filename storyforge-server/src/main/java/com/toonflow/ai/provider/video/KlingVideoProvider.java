...
public class KlingVideoProvider extends AbstractVideoProvider {

    private static final String DEFAULT_BASE_URL = "https://api.klingai.com";

    public KlingVideoProvider(Config config, OkHttpClient httpClient, ObjectMapper objectMapper) {
        super(config, httpClient, objectMapper);
    }

    @Override
    public String createTask(VideoRequest request) {
        String url = getBaseUrl(DEFAULT_BASE_URL) + "/v1/videos/generations";
    ...
    public VideoTaskResult pollTask(String taskId) {
        String url = getBaseUrl(DEFAULT_BASE_URL) + "/v1/videos/generations/" + taskId;
    ...
    private String doPostAndGetTaskId(String url, ObjectNode body) {
        try {
            ...
            try (Response resp = httpClient.newCall(req).execute()) {
                ...
                if (!resp.isSuccessful()) throw new BizException(ErrorCode.AI_CALL_FAILED, "Kling video HTTP " + resp.code());
                JsonNode root = objectMapper.readTree(respBody);
                return root.path("data").path("task_id").asText();
            }
        } catch (BizException e) { throw e; }
        catch (IOException e) { throw new BizException(ErrorCode.AI_CALL_FAILED, "Kling video error: " + e.getMessage()); }
    }
    ...
    private VideoTaskResult doPollGet(String url, String taskId) {
        try {
            ...
            try (Response resp = httpClient.newCall(req).execute()) {
                ...
                JsonNode videos = root.path("data").path("task_result").path("videos");
                if (videos.isArray() && !videos.isEmpty()) {
                    videoUrl = videos.get(0).path("url").asText(null);
                }
            ...
            throw new BizException(ErrorCode.AI_CALL_FAILED, "Kling video poll error: " + e.getMessage());
        }
    }

    private String mapStatus(String s) {
        return switch (s) { case "succeed" -> "completed"; case "failed" -> "failed"; case "processing" -> "processing"; default -> "pending"; };
    }
}
