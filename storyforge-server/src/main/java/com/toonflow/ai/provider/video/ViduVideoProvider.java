...
public class ViduVideoProvider extends AbstractVideoProvider {

    private static final String DEFAULT_BASE_URL = "https://api.vidu.com";

    public ViduVideoProvider(Config config, OkHttpClient httpClient, ObjectMapper objectMapper) {
    ...
    private String doPostAndGetTaskId(String url, ObjectNode body) {
        try {
            ...
            try (Response resp = httpClient.newCall(req).execute()) {
                ...
                if (!resp.isSuccessful()) throw new BizException(ErrorCode.AI_CALL_FAILED, "Vidu video HTTP " + resp.code());
                JsonNode root = objectMapper.readTree(respBody);
                return root.path("data").path("task_id").asText();
            }
        } catch (BizException e) { throw e; }
        catch (IOException e) { throw new BizException(ErrorCode.AI_CALL_FAILED, "Vidu video error: " + e.getMessage()); }
    }
    ...
    private VideoTaskResult doPollGet(String url, String taskId) {
        try {
            ...
            try (Response resp = httpClient.newCall(req).execute()) {
                String respBody = resp.body() != null ? resp.body().string() : "";
                JsonNode root = objectMapper.readTree(respBody);
                String status = root.path("state").asText("pending");
            ...
            throw new BizException(ErrorCode.AI_CALL_FAILED, "Vidu video poll error: " + e.getMessage());
        }
    }

    private String mapStatus(String s) {
        return switch (s) { case "completed", "success" -> "completed"; case "failed" -> "failed"; case "processing" -> "processing"; default -> "pending"; };
    }
}
